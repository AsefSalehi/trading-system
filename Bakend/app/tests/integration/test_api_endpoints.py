...
class TestCryptocurrencyEndpoints:
    ...
    async def test_list_cryptocurrencies_with_filters(
        ...
        response = await client.get("/api/v1/cryptocurrencies/?min_market_cap=500000000000")
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_list_cryptocurrencies_sorting(
        ...
        response = await client.get("/api/v1/cryptocurrencies/?sort_by=market_cap&order=desc")
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_sync_cryptocurrency_data_success(self, client: AsyncClient):
        ...
        with patch.object(cryptocurrency_service, 'fetch_and_store_listings') as mock_fetch:
            mock_fetch.return_value = [Cryptocurrency(id=1, symbol="BTC", name="Bitcoin", slug="bitcoin")]
            
            response = await client.post("/api/v1/cryptocurrencies/sync?limit=10&provider=coingecko")
...
class TestAPIValidation:
    ...
    async def test_price_history_invalid_dates(
        ...
        response = await client.get("/api/v1/cryptocurrencies/BTC/history?start_date=invalid-date")
        assert response.status_code == 422
...
class TestErrorHandling:
    ...
    async def test_database_error_handling(self, client: AsyncClient):
        ...
        with patch('app.services.cryptocurrency_service.cryptocurrency_service.get_cryptocurrencies') as mock_get:
