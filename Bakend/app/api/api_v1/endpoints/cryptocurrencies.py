...
async def list_cryptocurrencies(
    ...
    min_volume: Optional[float] = Query(None, ge=0, description="Minimum volume filter"),
...
async def get_cryptocurrency(
    ...
    """
    try:
        cryptocurrency = await cryptocurrency_service.get_cryptocurrency_by_symbol(db, symbol)
...
async def sync_cryptocurrency_data(
    request,  # Required for rate limiting
    limit: int = Query(100, ge=1, le=1000, description="Number of cryptocurrencies to sync"),
    provider: str = Query("coingecko", pattern="^(coingecko|coinmarketcap)$", description="Data provider"),
    db: AsyncSession = Depends(get_db)
):
    """
    ...
    try:
        ...
        logger.info(f"Completed crypto data sync: {updated_count} cryptocurrencies processed")
...
async def get_top_cryptocurrencies(
    request,  # Required for rate limiting
    category: str,
    limit: int = Query(10, ge=1, le=100, description="Number of top cryptocurrencies to return"),
    db: AsyncSession = Depends(get_db)
):
    """
