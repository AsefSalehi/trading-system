...
class CryptocurrencyService:
            ...
            for coin_data in crypto_data:
                try:
                    ...
                    if existing_crypto:
                        ...
                        await self._update_cryptocurrency(db, existing_crypto, coin_data)
                    ...
                    logger.error(f"Error processing coin {coin_data.get('symbol', 'unknown')}: {e}")
                    continue
            
            await db.commit()
            logger.info(f"Successfully processed {len(stored_cryptos)} cryptocurrencies")
    ...
    async def _update_cryptocurrency(
        ...
        try:
            ...
            crypto.price_change_percentage_24h = coin_data.get("price_change_percentage_24h")
            crypto.price_change_percentage_7d = coin_data.get("price_change_percentage_7d")
            crypto.price_change_percentage_30d = coin_data.get("price_change_percentage_30d")
            
            # Update ATH/ATL if they exist and are more recent
            if coin_data.get("ath") and (not crypto.ath or coin_data["ath"] > crypto.ath):
                crypto.ath = coin_data["ath"]
                crypto.ath_date = coin_data.get("ath_date")
            
            if coin_data.get("atl") and (not crypto.atl or coin_data["atl"] < crypto.atl):
    ...
    async def get_cryptocurrencies(
        ...
        try:
            ...
            # Apply sorting
            sort_column = getattr(Cryptocurrency, sort_by, Cryptocurrency.market_cap_rank)
            if order == "desc":
